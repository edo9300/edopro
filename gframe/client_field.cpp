...
void ClientField::GetCardDrawCoordinates(ClientCard* pcard, irr::core::vector3df* t, irr::core::vector3df* r, bool setTrans) {
	...
	if(location != LOCATION_HAND) {
		...
		if(location == LOCATION_MZONE || (location == LOCATION_SZONE) && ((pcard->type & TYPE_MONSTER && !(pcard->type & (TYPE_SPELL | TYPE_TRAP))) || pcard->type & TYPE_TRAPMONSTER) && !pcard->equipTarget) {			
		////////kdiy///////////	
			if(controler == 0)
		...
		if((location != LOCATION_GRAVE) && ((location == LOCATION_DECK && deck_reversed == pcard->is_reversed) ||
			...
			*r += facedown;
			...
			if((location == LOCATION_MZONE || (location == LOCATION_SZONE) && ((pcard->type & TYPE_MONSTER && !(pcard->type & (TYPE_SPELL | TYPE_TRAP))) || pcard->type & TYPE_TRAPMONSTER) && !pcard->equipTarget) && pcard->position & POS_DEFENSE)
			////////kdiy///////////
