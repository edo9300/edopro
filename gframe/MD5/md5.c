...
/* MD5 block update operation. Continues an MD5 message-digest
  ...
  context.
 */
void MD5Update(MD5_Context* context, const void* input, size_t inputLen)
{
  ...
  */
	if (inputLen >= partLen) {
		...
		for (i = partLen; i + 63 < inputLen; i += 64)
			MD5Transform(context->state, &((POINTER)input)[i]);
	...
	memcpy(&context->buffer[index], &((POINTER)input)[i],
		inputLen - i);
}
