...
bool SoundManager::PlayChant(CHANT chant, uint32_t code, uint32_t code2, uint8_t player) {
///////kdiy//////
#ifdef BACKEND
	if(!soundsEnabled) return false;
	///////kdiy//////
	int acharacter = character;
	if (player==1) acharacter = character2;
	if(code == 0) {
		int i=-1;
