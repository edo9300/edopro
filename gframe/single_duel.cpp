...
void SingleDuel::TPResult(DuelPlayer* dp, unsigned char tp) {
	...
	if(!host_info.no_shuffle_deck) {
		std::shuffle(pdeck[0].main.begin(), pdeck[0].main.end(), rnd);
		std::shuffle(pdeck[1].main.begin(), pdeck[1].main.end(), rnd);
	}
	...
	if(host_info.extra_rules & DUEL_SPEED)
		opt |= SPEED_DUEL;
	new_replay.WriteInt32((mainGame->GetMasterRule(opt & ~SPEED_DUEL, 0)) | (opt & SPEED_DUEL) << 8);
...
int SingleDuel::Analyze(char* msgbuffer, unsigned int len) {
	...
	while (pbuf - msgbuffer < (int)len) {
		...
		ReplayPacket p(engType, pbuf, len - 1);
		switch (engType) {
		case MSG_RETRY: {
		...
		case MSG_MOVE: {
			...
			pbuf += 4;
			NetServer::SendBufferToPlayer(players[current.controler], STOC_GAME_MSG, offset, pbuf - offset);
			...
			NetServer::SendBufferToPlayer(players[1 - current.controler], STOC_GAME_MSG, offset, pbuf - offset);
			for(auto oit = observers.begin(); oit != observers.end(); ++oit)
				NetServer::ReSendToPlayer(*oit);
...
void SingleDuel::RefreshMzone(int player, int flag, int use_cache) {
	...
	NetServer::SendBufferToPlayer(players[1 - player], STOC_GAME_MSG, buffer.data(), buffer.size());
	for(auto pit = observers.begin(); pit != observers.end(); ++pit)
		NetServer::ReSendToPlayer(*pit);
}
