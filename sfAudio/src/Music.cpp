...
{
    Span<uint64_t> samplePoints(timeToSamples(timePoints.offset), timeToSamples(timePoints.length));
    ...
    {
        std::cerr << "Music is not in a valid state to assign Loop Points." << std::endl;
        return;
    }
...
////////////////////////////////////////////////////////////
uint64_t Music::timeToSamples(Time position) const
{
    // Always ROUND, no unchecked truncation, hence the addition in the numerator.
    // This avoids most precision errors arising from "samples => Time => samples" conversions
    // Original rounding calculation is ((Micros * Freq * Channels) / 1000000) + 0.5
    // We refactor it to keep int64_t as the data type throughout the whole operation.
    return ((position.asMicroseconds() * getSampleRate() * getChannelCount()) + 500000) / 1000000;
}


////////////////////////////////////////////////////////////
Time Music::samplesToTime(uint64_t samples) const
{
